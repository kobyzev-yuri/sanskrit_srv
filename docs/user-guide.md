...
| Фильтр | **только несогласованные** | Скрывает страницы со статусом «согласовано»; счётчик `N / всего`. Стрелки ‹ › тоже ходят по отфильтрованному списку |
...
Shift+клик по любой из кнопок — пересобрать файл заново (иначе отдаётся последний собранный).
...
Симптом: на скане **गणपतिगुंँ हवामहे**, в черновике **गणपतिं हवामहे** — модель подставила «словарное» **गणपतिं**. Редкое **ऋतग्म्** рядом часто не трогает, потому что в памяти нет «правильной» формы.

Так же со случайными **॑/॒**: акценты только там, где штрих есть на пластине.

Оставляйте орфографию скана. Если уже испортилось:

> получилось गणपतिं हवामहे исправь на गणपतिगुंँ हवामहे
...
### UC-1f. Ведийские тоны (॑ ॒) пропали или «подчёркнуто всё слово»

Симптом: на скане мантр с svara, в превью тонов нет; или под слогами сплошная черта / кракозябры.

| Нужно | Unicode | Вид |
|-------|---------|-----|
| Чёрточка **сверху** | **॑** U+0951 | после слога: `य॑` |
| Чёрточка **снизу** (anudātta) | **॒** U+0952 | после слога: `य॒` |

Несколько anudātta подряд на скане выглядят как прерывистое подчёркивание — так и должно быть. Не заменять на HTML-подчёркивание.

1. **Справочник** → блок **Ведийские тоны** → копировать **॑** / **॒** или готовый слог.
2. Задание, например:

   > расставь тоны ॑ и ॒ по скану; получилось यज्ञेन исправь на य॒ज्ञेन॑

3. **Отправить задание** / **Пересмотри страницу**. Плохой скан → точечная ручная правка в HTML неизбежна.

...
| **expert** | Проекты, редактор, **Кабинет** (логин / email / пароль, свои ключи LLM) |
...
| **Кабинет** | Логин, email, пароль; свои ключи OpenRouter / ProxyAPI и сеть; расход по своим вызовам. «Вернуться к токенам бэкофиса», если администратор их назначил |
| **Бэкофис** | Только admin: пользователи, **маршрут LLM** (Gemini Pro / OpenRouter / Opus), каталог моделей, расход по проектам и по ключам |
...
| **Поиск** | поле **Найти в черновике** рядом с пейджером (Ctrl+F) | Подстрока по черновикам всей книги (в переводе — ещё и санскрит слева). Список совпадений под полем; клик открывает страницу. Esc — сбросить |
...
| **Перевести все** | полоса шаблона сверху (перевод на русский) | Массовый LLM-перевод. Тот же фильтр: несогласованные или вся книга. На панели конкретной страницы этой кнопки нет — там только «Перевести страницу» |
| **Смысловая проверка всех** | кнопка (перевод на русский) | Второй проход по уже переведённым страницам: грубые обрывы перевода исправляются, тонкие замечания (стык страниц, санскрит, смысл) помечаются на миниатюре |
...
| Панель **Скан** / **Санскрит** | слева | Оцифровка: страница PDF. Перевод: выверенный санскрит (**Превью** / **HTML**). Правка HTML санскрита пишется в проект оцифровки **новой версией** |
| **Превью** / **Текст** / **HTML** | центр (перевод) или справа (оцифровка) | Черновик страницы; сохранить и согласовать здесь же, без подвала поверх текста |
...
**Оцифровка:** слева скан, справа превью (вкладки **Превью** / **Текст** / **HTML**). Задание модели и **Справочник** — отдельная панель под превью, чтобы черновик не прятался за подвалом кнопок.

**Перевод на русский:** три колонки — слева выверенный санскрит (**Превью** / **HTML**), в центре превью/текст/HTML перевода и кнопки сохранить/согласовать, справа **Задание** (агент, смысловая проверка) и **Справочник**. Правка санскрита слева (вкладка **HTML** или смысловая проверка «слева») копируется в связанную страницу оцифровки: сначала снимок текущего HTML, затем новая версия. Если страница оцифровки уже была согласована, согласие снимается.

| Слева | Центр (перевод) / справа (оцифровка) | Справа (перевод) |
|-------|--------------------------------------|------------------|
| **Скан** или **Санскрит** (+ **HTML** в переводе) | Вкладки **Превью** / **Текст** / **HTML** | **Задание** и **Справочник** |
...
Чтобы пройти **всю книгу**: тот же фильтр миниатюр (**только несогласованные** / все) и кнопка **Смысловая проверка всех**. На несогласованных грубые обрывы правятся сразу; тонкие замечания остаются в списке (на миниатюре — число). На уже согласованных правки сами не пишутся: отзовите согласие и примените выбранные.

Не путать с **Перевести все** — то полный перевод заново.

...
| **Слить переводы** | Только перевод: внизу центральной колонки поле второго варианта (профессор / другая модель), даже одна шлока; модель соберёт гибрид с текущим черновиком |
...
### UC-9. Два перевода одной шлоки (Gemini и профессор)

Когда на странице уже есть черновик, а под рукой другой перевод той же шлоки (или всей страницы):

1. Страница **на правке** (если согласована — сначала **Отозвать согласие**).
2. Внизу центральной колонки (под превью, над «Сохранить») — поле **Второй перевод**. Вставьте текст профессора (можно без HTML, можно одну шлоку с ॥ n ॥).
3. **Слить переводы**. Модель не выбирает целиком A или B: берёт грамматику и термины более точного варианта и оставляет читаемую русскую фразу в формате шаблона. Одна шлока заменяет только свою пару деванагари + русский.
4. Сверить превью → при необходимости **Смысловая проверка** или правка вручную → **Сохранить и согласовать**.
